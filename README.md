...
**Q:数字和官方网页对不上?**
以智谱开放平台「个人编程套餐 > 用量统计」网页为权威;本插件数字来自同一个监控接口,字段若被官方调整,更新脚本中的映射即可(集中在 `zcode-watch.mjs` 一处)。

**Q:每月 1 号怎么重置的?**
查询窗口天然是「当月 1 日 00:00 → 现在」;缓存文件里的逐日高峰数据在新月份自动清空,无需手动操作。

**Q:月初/断档后第一次刷新怎么有点慢?**
用量按 ≤7 天分段拉取逐小时序列;断档后首刷会自动补齐缺口(整月一次性 ≤5 个请求/Key),之后稳态每次只要 2 个请求/Key(档位 + 昨天到现在的滚动窗口),已结算的日期全部走缓存。
